function [Cpif,xcif] = CPvsNormC(alph)
%function takes in desired angle of attack, returns data for Cp vs
%normalized chord at angle and plots the data 
%% Read In Data 
...
Px16 = SpanTest(:,30); %Scanivalve Pressure 16 [Pa]
    ...
    C =max(PortLoc(:,3)); % [in] % Z is along cord I believe based on data range 0:about 3 in 
...
% focusing on 4* angle of attack: row alphInd of data table

alphInd = find(alpha==alph,1);

%interpolate last data point 
%Cpf = interp1(xci(8:9), Cpi(alphInd,8:9),0.9)
slope = (Cpi(alphInd,8)-Cpi(alphInd,9))  / (xci(8)-xci(9));
Cpf = slope*(1-xci(8))+Cpi(alphInd,8);

% creating a single array 
xcif = [xci(1:9), 1, xci(10:16)];
Cpif = [Cpi(alphInd,1:9), Cpf,Cpi(alphInd,10:16) ];
...
% plot(xcif, Cpif,['.-' ]);
% plot(1,Cpf);
% title("Normalized Chord vs Coeff. of Pressure at alpha ="+alph+' degrees')
% xlabel('X/c');
% ylabel("Cp")
